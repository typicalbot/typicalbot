...
export default class extends Command {
    ...
    async execute(message: TypicalGuildMessage, parameters: string) {
        if (!await this.client.utility.music.hasPermissions(message, message.guild.settings.music.unqueue)) return;
        ...
        try {
            ...
            if (!results.length) return message.error(message.translate('unqueue:NONE', { query }));

            if (results.length < 1) {
                const index = queue.findIndex(video => video.title === results[0].title);
            ...
            message.send([
                message.translate('unqueue:FOUND_MULTIPLE'),
                '',
                videos
            ].join('\n'));

            const messages = await message.channel.awaitMessages((m) => m.author.id === message.author.id, { max: 1, time: 10000, errors: ['time'] }).catch(() => null);

            if (!messages || !messages.size) return message.error(message.translate('common:NO_RESPONSE'));
            ...
            if (first.content === 'all') {
                ...
                return message.reply(message.translate('unqueue:ALL', { amount: results.length }));
            }
            if (indexRegex.test(first.content)) {
                queue.splice(queue.indexOf(results[parseInt(first.content, 10) - 1]), 1);
