...
module.exports = class extends Command {
    ...
    async execute(message, parameters, permissionLevel) {
        const args = /(?:<@!?)?(\d{17,20})>?(?:\s+(?:(\d+)d(?:ays?)?)?\s?(?:(\d+)h(?:ours?|rs?)?)?\s?(?:(\d+)m(?:inutes?|in)?)?\s?(?:(\d+)s(?:econds?|ec)?)?)?(?:\s*(\d+))?(?:\s*(.+))?/i.exec(parameters);
        if (!args) return message.error(this.client.functions.error('usage', this));

        const user = args[1]; const days = args[2]; const hours = args[3]; const minutes = args[4]; const seconds = args[5]; const purgeDays = args[6] || 0; const
            reason = args[7];
        ...
        this.client.users.fetch(user).then(async (cachedUser) => {
            const member = await message.guild.members.fetch(cachedUser).catch((err) => { });
