...
module.exports = class extends Command {
    ...
    async execute(message, parameters) {
        ...
        const id = args[1]; const
            reason = args[2];

        this.client.handlers.moderationLog.fetchCase(message.guild, id).then((log) => {
            if (!log) return message.error('There are no moderation logs under the given case ID.');

            this.client.handlers.moderationLog.edit(log, message.author, reason).then(
