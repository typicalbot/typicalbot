...
export default class extends Command {
    ...
    execute(message: Message) {
        if (message.channel.type === 'text') message.reply(message.translate('commands:CHECK_DM'));
        ...
        return message.author && message.author.send(new MessageEmbed()
            .setColor(0x00ADFF)
            .setTitle('TypicalBot Commands')
            .setDescription(message.translate('commands:TEXT', { prefix: this.client.config.prefix }))
            .addField(message.translate('commands:OWNER'), level4.length ? level4.join(', ') : NONE)
            .addField(message.translate('commands:ADMIN'), level3.length ? level3.join(', ') : NONE)
            .addField(message.translate('commands:MOD'), level2.length ? level2.join(', ') : NONE)
            .addField(message.translate('commands:DJ'), level1.length ? level1.join(', ') : NONE)
            .addField(message.translate('commands:MEMBER'), level0.length ? level0.join(', ') : NONE)
