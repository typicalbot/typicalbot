...
export default class extends Command {
    ...
    async execute(message: TypicalGuildMessage, parameters: string) {
        ...
        const type = message.translate(reset ? 'common:RESET' : 'common:CHANGED').toLowerCase();

        // If a member was not found but a user id was provided cancel
        if (!member && userID) return null;
        // A user id was not found and so edit the authors nickname
        if (!member) {
            if (message.guild.settings.nonickname) return message.error('nickname:DISABLED');

            const changed = await message.member.setNickname(reset ? '' : nickname).catch(() => null);
            if (!changed) return message.error(message.translate('nickname:SELF_ERROR'));
            return message.reply(message.translate('nickname:SELF_SUCCESS', { type }));
        }
        ...
        if (permissions.level < 2) return message.error(this.client.helpers.permissionError.execute(message, this, permissions));

        const changed = await member.setNickname(reset ? '' : nickname).catch(() => null);
