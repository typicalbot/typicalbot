...
const regex = /(\d+)?/i;
const replaceFirstRegex = /[^a-z0-9]/gmi;
...
export default class extends Command {
    ...
    async execute(message: TypicalGuildMessage, parameters: string) {
        ...
        const bots = data
            .filter((bot) => parseInt(bot.botid, 10) > 10 && parseInt(bot.servercount, 10) > 0)
            .sort((a, b) => parseInt(b.servercount, 10) - parseInt(a.servercount, 10))
            .map(bot => ({
                ...bot,
                name: this.name.replace(replaceFirstRegex, '').replace(replaceSecondRegex, ''),
                servercount: Number(bot.servercount).toLocaleString()
            }));

        const content = this.client.helpers.pagify.execute(
            bots.map((bot) => `${this.client.helpers.lengthen.execute(bot.name, 20)}: ${bot.servercount}${bot.compliant ? message.translate('bots:COMPLIANT') : ''}`),
            page,
        );
