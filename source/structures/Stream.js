class Stream {
    ...
    play(video) {
        this.client.audioUtility.fetchStream(video).then(audioStream => {
            ...
            video.response.send(`🎵 Now streaming **${video.title}** requested by **${video.response.message.author.username}** for **${this.client.functions.convertTime.execute(video.length_seconds * 1000)}**.`);
