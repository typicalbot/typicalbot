...
class PermissionsHandler {
    constructor(client) {
        ...
        this.levels = {
            ...
            [Constants.Permissions.Levels.SERVER_BLACKLISTED]: new PermissionLevel(Constants.Permissions.Levels.SERVER_BLACKLISTED, "Server Blacklisted", (guild, member) => {
            ...
            [Constants.Permissions.Levels.SERVER_MEMBER] : new PermissionLevel(Constants.Permissions.Levels.SERVER_MEMBER, "Server Member"),
            [Constants.Permissions.Levels.SERVER_DJ] : new PermissionLevel(Constants.Permissions.Levels.SERVER_DJ, "Server DJ", (guild, member) => {
                const role = this.fetchRoles(guild, "dj"); if (!role) return;
                if (role instanceof Array) { let isRole = false; role.forEach(r => { if (member.roles.has(r.id)) isRole = true; }); return isRole; } else { return member.roles.has(role.id); }
            }),
            [Constants.Permissions.Levels.SERVER_MODERATOR] : new PermissionLevel(Constants.Permissions.Levels.SERVER_MODERATOR, "Server Moderator", (guild, member) => {
                const role = this.fetchRoles(guild, "moderator"); if (!role) return;
                if (role instanceof Array) { let isRole = false; role.forEach(r => { if (member.roles.has(r.id)) isRole = true; }); return isRole; } else { return member.roles.has(role.id); }
            }),
            [Constants.Permissions.Levels.SERVER_ADMINISTRATOR] : new PermissionLevel(Constants.Permissions.Levels.SERVER_ADMINISTRATOR, "Server Administrator", (guild, member) => {
