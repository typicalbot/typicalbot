...
module.exports = class extends Command {
    ...
    async execute(message, parameters, permissionLevel) {
        ...
        if (action === "list") {
            message.reply(`Current aliases:\n${message.guildSettings.aliases.length ? message.guildSettings.aliases.map(a => `${a.alias} -> ${a.command}`).join("\n") : "None"}`);
        } else if (action === "add") {
            ...
            const aliasList = message.guildSettings.aliases;
            const aliasListF = message.guildSettings.aliases.map(a => a.alias);
