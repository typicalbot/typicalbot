...
module.exports = class extends Command {
    ...
    execute(message, permissionLevel) {
        ...
        if (!user || user.id === message.author.id) return message.send(`${message.author} just shot at themselves! :scream:${randomAddon || ""}`);
        message.send(`${message.author} just shot at ${user}! :scream:${randomAddon || ""}`);
    }
};
