...
module.exports = class extends Command {
    ...
    execute(message, response, permissionLevel) {
        const args = /unignore\s+(channels|invites)/i.exec(message.content);
        ...
        if (invites && !message.guild.settings.ignored.commands.includes(message.channel.id)) return response.error("This channel isn't ignoring invites.");
        ...
        this.client.settings.update(message.guild.id, { ignored: { [commands ? "commands" : "invites"]: newArray }}).then(() => response.success("Setting successfully updated."));
    }
};
