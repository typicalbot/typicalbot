...
module.exports = class extends Command {
    ...
    async execute(message, response, permissionLevel) {
        ...
        const user = args[1], days = args[2], reason = args[3];
        ...
        this.client.users.fetch(user).then(async cachedUser => {
            ...
            if (!member.bannable) return response.error(`In order to complete the request, I need the **BAN_MEMBERS** permission and my highest role needs to be the requested user's highest role.`);
                ...
                setTimeout(() => {
                    message.guild.unban(actioned.id).then(async () => {
                        if (message.guild.settings.logs.moderation) {
                            ...
                            this.client.softbanCache.set(actioned.user.id || actioned.user, log);
