    ...
    async execute(message, response, permissionLevel) {
        ...
        if (subcommand === "help") {
            ...
            const content = this.client.functions.get("pagify").execute(
                message.guild.roles.filterArray(r => r.position !== 0).sort((a,b) => b.position - a.position).map(r => `${this.client.functions.get("lengthen").execute(1, r.name, 30)} : ${r.id}`),
                args2 || 1
            );
            ...
            if (subAction === "members") {
                ...
                const content = this.client.functions.get("pagify").execute(
                    subRole.members.map(m => `${this.client.functions.get("lengthen").execute(1, m.user.username, 30)} : ${m.id}`),
                    subPage || 1
                );
            ...
            const subMember = await this.client.functions.get("resolveMember").execute(message, subArgs, false);
            ...
            if (subAction === "list") {
                ...
                const content = this.client.functions.get("pagify").execute(
                    roleList.sort((a,b) => b.position - a.position).map(r => `${this.client.functions.get("lengthen").execute(1, r.name, 30)} : ${r.id}`),
                    subArgs[3] || 1
                );
