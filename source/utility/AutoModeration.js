    ...
    inviteCheck(response) {
        return new Promise((resolve, reject) => {
            if (response.message.guild.settings.automod.invite) {
                const contentMatch = this.client.functions.inviteCheck.execute(response.message.content);
                const embedMatch = this.client.functions.inviteCheck.execute(inspect(response.message.embeds, { depth: 4 }));
