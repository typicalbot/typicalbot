...
class New extends Event {
    ...
    async execute(oldMember, member) {
        ...
        if (settings.auto.nickname && nickname === this.client.functions.formatMessage.execute("jn", guild, user, settings.auto.nickname)) return;

        channel.send(
            settings.logs.nickname !== "--enabled" ?
                this.client.functions.formatMessage.execute("ann-nick", guild, user, settings.logs.nickname, { oldMember }) :
