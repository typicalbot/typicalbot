...
module.exports = class {
    ...
    stream(response, video) {
        return new Promise((resolve, reject) => {
            if (!this.client.audioUtility.withinLimit(video, response)) return response.error(`The song requested is too long to play. The maximum song length is ${this.client.functions.get("length").execute(response.message.guild.settings.lengthLimit || 1800)}.`);
