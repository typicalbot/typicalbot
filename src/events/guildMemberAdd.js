...
class GuildMemberAdd extends Event {
    ...
    async execute(member) {
        ...
        if (settings.auto.message && !user.bot) user.send(`**${guild.name}'s Join Message:**\n\n${this.client.functions.formatMessage('automessage', guild, user, settings.auto.message)}`).catch(() => { });

        if (settings.auto.nickname) member.setNickname(this.client.functions.formatMessage('autonick', guild, user, settings.auto.nickname)).catch(() => { });

        let autorole;
        if (settings.auto.role.bots && member.user.bot) { autorole = settings.auto.role.bots ? guild.roles.has(settings.auto.role.bots) ? guild.roles.get(settings.auto.role.bots) : null : null; } else if (settings.auto.role.id) { autorole = settings.auto.role.id ? guild.roles.has(settings.auto.role.id) ? guild.roles.get(settings.auto.role.id) : null : null; }

        if (autorole && autorole.editable) {
            setTimeout(() => member.roles.add(autorole).then(() => {
