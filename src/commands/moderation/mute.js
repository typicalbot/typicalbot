...
module.exports = class extends Command {
    ...
    async execute(message, parameters, permissionLevel) {
        ...
        if (args[7]) {
            ...
            const user = args[1]; const days = args[2]; const hours = args[3]; const minutes = args[4]; const seconds = args[5]; const
                reason = args[6];
            ...
            this.client.users.fetch(user).then(async (cachedUser) => {
                ...
                const embed = cachedUser.buildEmbed().setColor(Constants.ModerationLog.Types.MUTE.hex).setFooter('TypicalBot', Constants.Links.ICON).setTitle('TypicalBot Alert System')
                ...
                member.roles.add(role).then(async (actioned) => {
                    if (message.guild.settings.logs.moderation) {
                        const newCase = this.client.handlers.moderationLog.buildCase(message.guild).setExpiration(time).setAction(Constants.ModerationLog.Types.MUTE).setModerator(message.author)
                            .setUser(member.user);
