...
module.exports = class extends Command {
    ...
    async execute(message, parameters) {
        ...
        if (url && url[1]) { this.client.handlers.music.stream(message, url[1], true).catch((err) => message.error(err.stack || err)); } else if (url && url[2]) {
            this.client.utility.music.fetchInfo(url[2], message).then(async (video) => {
                if (video.live && (await this.client.functions.fetchAccess(message.guild)).level < 1) { return message.error('This feature is only available to TypicalBot Prime members.'); }
            ...
            this.client.utility.music.search(message.guild.settings, args[1]).then((results) => {
                ...
                this.client.utility.music.fetchInfo(results[0].url, message).then((video) => this.client.handlers.music.stream(message, video).catch((err) => message.error(err.stack || err))).catch((err) => message.error(`Information cannot be fetched from that song. Please try another song name.${message.author.id === '105408136285818880' ? `\n\n\`\`\`${err}\`\`\`` : ''}`));
