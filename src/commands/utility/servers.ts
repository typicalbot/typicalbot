...
export default class extends Command {
    ...
    execute(message: TypicalGuildMessage) {
        ...
        const paged = this.client.helpers.pagify.execute(message, this.client.guilds.cache
            .array()
            .sort((a, b) => b.memberCount - a.memberCount)
            .map((g) =>
                `${this.client.helpers.lengthen.execute(`${g.name.replace(guildNameRegex, '')}`, 30)} : ${g.memberCount}`), page);
        ...
        if (!message.embeddable)
            return message.reply([CLUSTER, '```autohotkey', paged, '```'].join('\n'));
